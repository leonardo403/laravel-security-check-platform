...
<div class="min-h-screen flex items-center justify-center px-4 py-10">
    <div class="w-full max-w-md bg-slate-900/80 border border-slate-700 rounded-2xl shadow-2xl p-8">
        <p class="text-sm uppercase tracking-[0.3em] text-cyan-400 mb-3">{{ __('auth.register') }}</p>
        <h2 class="text-2xl font-bold mb-6">{{ __('auth.register_title') }}</h2>

        <form method="POST" action="/register" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium mb-1 text-slate-300">{{ __('common.name') }}</label>
            ...
            <div>
                <label class="block text-sm font-medium mb-1 text-slate-300">{{ __('auth.confirm_password') }}</label>
            ...
            <button type="submit" class="w-full rounded-xl bg-green-500 py-2.5 font-semibold text-slate-950 hover:bg-green-400">{{ __('auth.register') }}</button>
        </form>

        <p class="mt-6 text-sm text-slate-400">{{ __('auth.have_account') }} <a href="/login" class="text-cyan-400 hover:text-cyan-300">{{ __('auth.login') }}</a></p>
    </div>
</div>
@endsection
